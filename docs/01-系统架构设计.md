...
- 版本: v1.1
- 日期: 2026-04-19
- 状态: Phase 6 开发中（数据库已启用，记忆系统已接入，服务层已扩展）
...
flowchart TB
    ...
    subgraph 应用层[应用层 - Application Layer]
        ...
        ResultSvc[结果服务]
        StatsSvc[统计服务]
        HomeworkSvc[HomeworkService<br/>作业持久化服务]
        MemorySvc[MemoryService<br/>记忆查询服务]
    end
    ...
    subgraph 基础设施层[基础设施层 - Infrastructure Layer]
        ...
        subgraph 存储层[存储层]
            SQLite[(SQLite 活跃数据<br/>运行时自动建表)]
            Markdown[(Markdown 归档)]
            VectorDB[(向量数据库<br/>预留)]
        end
    ...
    HomeworkSvc --> SQLite
    MemorySvc --> ProfileMem
    MemorySvc --> SemanticMem
    MemorySvc --> MetaMem
