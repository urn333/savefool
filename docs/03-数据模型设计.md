...
> **变更记录**: Phase 6 扩展 Homework 模型（诊断字段、状态枚举），数据库已启用运行时代理建表
...
| status | VARCHAR(16) | NOT NULL | 'pending' | 状态(pending/processing/completed/failed/active/archived/deleted) |
| processed_image_url | VARCHAR(512) | - | NULL | 预处理后图片URL |
| parent_description | TEXT | - | NULL | 家长描述 |
| diagnosis_mode | VARCHAR(16) | - | NULL | 诊断模式(diagnosis/solution/explain/single) |
| diagnosis_result | JSON | - | NULL | 诊断结果JSON |
| error_count | INTEGER | NOT NULL | 0 | 错题数量 |
| total_count | INTEGER | NOT NULL | 0 | 总题数 |
| completed_at | DATETIME | - | NULL | 完成时间 |
| raw_model_response | TEXT | - | NULL | 模型原始响应 |
...
| 枚举值 | 说明 | 数据保留策略 |
|--------|------|--------------|
| `pending` | 待处理（上传完成，等待用户确认分析） | 完整保留 |
| `processing` | 处理中（AI诊断进行中） | 完整保留 |
| `completed` | 已完成（诊断完成） | 完整保留 |
| `failed` | 失败（诊断过程中出错） | 保留，允许重试 |
| `active` | 活跃（兼容旧数据） | 完整保留 |
...
stateDiagram-v2
    [*] --> pending: 上传作业
    pending --> processing: 用户确认分析
    processing --> completed: 诊断成功
    processing --> failed: 诊断出错
    failed --> pending: 用户重试
    completed --> archived: 30天后
    archived --> deleted: 用户删除/1年后
    active --> archived: 30天后 (兼容旧数据)
    active --> deleted: 用户删除
```
...
| HOMEWORK | 诊断完成时 | status → 'completed', 更新 error_count/total_count/diagnosis_result |
| HOMEWORK | 诊断失败时 | status → 'failed', 记录 error_message |
...
CREATE TABLE homework (
    ...
    status VARCHAR(16) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'processing', 'completed', 'failed', 'active', 'archived', 'deleted')),
    processed_image_url VARCHAR(512),
    parent_description TEXT,
    diagnosis_mode VARCHAR(16),
